...
@section('content')
    <form method="post" action="{{ route('instructor.update', ['instructor' => $instructor->id]) }}" class="row" autocomplete="off">
        ...
        <div class="form-group mb-3 col-md-6">
            ...
            <input type="text" id="name" name="name" class="form-control" value="{{ $instructor->name }}">
        </div>
        
        <div class="form-group mb-3 col-md-6">
            <label for="email">Email</label>
            <input type="text" id="email" name="email" class="form-control" value="{{ $instructor->email }}">
        </div>
